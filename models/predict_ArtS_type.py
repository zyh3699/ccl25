...
2.  **重点关注否定形式**：
    ...
    *   **特殊谓词处理**：对于“证明”、“证实”这类谓词，如果背景句是“X **没有** 证明/证实 Y”的形式，这表示 **缺乏证据**，Y 的真假 **无法确定**。在这种特定情况下，请直接判断为 **U**。
3.  **推理步骤**：
    a.  仔细分析背景句，识别谓词 "{predicate}" 以及它前面是否有否定词。
    b.  根据上述原则，确定背景句所 **预设或隐含的事实**。特别注意否定形式下的事实预设。
    c.  检查是否属于“没有证明/证实”的特殊情况。如果是，直接判断为 U。
    d.  如果不是特殊情况，将提取出的事实与结论句 "{hypothesis}" 进行比较。
    e.  如果结论句与事实一致，则判断为 T。
    f.  如果结论句与事实矛盾，则判断为 F。
